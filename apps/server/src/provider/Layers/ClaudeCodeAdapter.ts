...
interface ClaudeSessionContext {
  /** The original threadId supplied by the caller at startSession time (used as map key). */
  ...
  return Effect.gen(function* () {
    ...
    const ensureThreadId = (
      ...
      Effect.gen(function* () {
        ...
        // Promote the SDK session_id to the session threadId when not yet set
    ...
    const resolveSession = (
      ...
      if (threadId !== undefined) {
        ...
        // Fallback: match by session.threadId (e.g. resume-cursor derived threadId)
        for (const ctx of sessions.values()) {
          if (ctx.session.threadId === threadId) return ctx;
        }
      }
      // When threadId is undefined, find any active session (pre-SDK-session-id state)
      if (threadId === undefined) {
        for (const ctx of sessions.values()) {
    ...
    const startSession: ClaudeCodeAdapterShape["startSession"] = (input) =>
      Effect.gen(function* () {
        ...
        const resolvedThreadId = resumeState?.threadId;

        const session: ProviderSession = {
          threadId: resolvedThreadId as unknown as ThreadId,
          provider: PROVIDER,
          status: "ready",
